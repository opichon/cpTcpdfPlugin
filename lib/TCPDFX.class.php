...
//    $this->setFooterFont(array($this->footer_font_family, $this->footer_font_style, $this->footer_font_size));
  ...
  */
  ...
  public function printSubTitle($text) {
    $this->SetFont($this->subtitle_font, $this->subtitle_style, $this->subtitle_size);
    $this->MultiCell(0, $this->subtitle_cell_height, $text, 0, $this->subtitle_align, 0, 1);
    $this->resetFont();
  }
...
//    $this->SetFont($font, $style, $size);
   ...
   * - 'fill' => whether the cells should be painted (true) or transparent (false).
   * - 'html' => whether the cell's content is HTML markup (true) or plain text (false).
   * @param Array footers an array of arrays containing the footer information. Each array
   * defines a footer row starting fromthe top (0 => first footer row, etc.). Each footer
   * row array is an associative array. The same keys are used as for the header row.
   * @param mixed fill defines whether the cells' background should be painted (true) or transparent (false). If a single value,
   * it applies to all cells in the row; otherwise an array with a value for each cell in the row.
   * @param mixed html defines whether the cells' content is HTML markup (true) or plain text (false).
...
//        $this->SetX($x0);
  ...
  protected function __row($data, $width, $align = 'C', $border, $font, $font_style, $font_size, $cell_height, $min_height = 0, $height = 0, $fill = 0, $html = 0) {
    $x0 = $this->GetX();
    $y0 = $this->GetY();
    ...
    for ($i = 0; $i < $n; $i++) {
      ...
      $ishtml = is_array($html) ? $html[$i] : $html;
      if ($ishtml) {
        $this->writeHTMLCell(is_array($width) ? $width[$i] : $width,
                         $height,
                         $this->GetX(),
                         $this->GetY(),
                         $data[$i],
...
//    $cell_height = round(($this->getCellHeightRatio() * $headerfont[2]) / $this->getScaleFactor(), 2);
    ...
    $this->SetY($this->paginate_y ? $this->paginate_y : max($this->getHeaderMargin(), $this->GetY()), false);

    if ($this->getRTL()) {
      $this->Cell(0, 0, $this->getPagination(), 0, 0, 'L');
    }
    else {
      $this->Cell(0, 0, $shift = $this->getAliasRightShift() . $this->getPagination(), 0, 0, 'R');
    }
    $this->resetFont();
  }
